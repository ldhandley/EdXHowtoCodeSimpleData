...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname space-invaders-043019) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define-struct game (invaders missiles tank timer))
;; Game is (make-game  (listof Invader) (listof Missile) Tank Integer)
;; interp. the current state of a space invaders game
;;         with the current invaders, missiles and tank position
;;         and with a starting time to put on the timer
...
;; Game -> Game
;; start the world with (main (make-game empty empty T0 0))
;; 
(define (main g)
  (big-bang g                   ; Game
            (on-tick   tock GAME-RATE)     ; Game -> Game
...
;;(define (tock game) (make-game empty empty T0 0));stub
(check-expect (tock (make-game empty empty (make-tank 50 1) 0))
              (make-game empty empty (move-tank (make-tank 50 1)) (+ 0 GAME-RATE)))
...
(define (tock g)
  (make-game
   ...
   (time-passes (game-timer g))))

;; Number -> Number
;; Makes the clock pass by GAME-RATE
;;(define (time-passes t) 0);stub
(check-expect (time-passes 0) (+ 0 GAME-RATE))
(check-expect (time-passes 2) (+ 2 GAME-RATE))
(check-expect (time-passes 1.6) (+ 1.6 GAME-RATE))
(define (time-passes t)
  (+ t GAME-RATE))
...
(check-expect (render (make-game empty empty T0 0))
              (render-timer (make-game empty empty T0 0)
                            (place-image TANK
                                         (tank-x T0)
                                         (- HEIGHT TANK-HEIGHT/2)
                                         BACKGROUND)))
(check-expect (render (make-game empty empty (make-tank (/ WIDTH 4) -1) 0))
              (render-timer (make-game empty empty T0 0)
                            (place-image TANK
                                         (/ WIDTH 4)
                                         (- HEIGHT TANK-HEIGHT/2)
...
(define (render g) 
   (render-timer g (render-invaders g (render-missiles g (render-tank g BACKGROUND)))))

;; Game Image -> Image
;; grabs the timer and places timer on background
;;(define (render-timer g img) BACKGROUND);stub
(check-expect (render-timer G0 BACKGROUND)
              (place-image/align (text (string-append (number->string 0) " " "sec") TEXT-SIZE TEXT-COLOR)
                                 (- WIDTH 20) (+ 0 20) "right" "top"
                                 BACKGROUND))
(define (render-timer g img)
  (place-image/align (text (string-append (number->string (round (game-timer g))) " " "sec") TEXT-SIZE TEXT-COLOR)
                     (- WIDTH 20) (+ 0 20) "right" "top"
                     img))
